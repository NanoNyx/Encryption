...
class Scrambler:
    ...
    def benchmark(self, its):
        ...
        print("\n\nЗначення X^2 =", hi_sqr)
        if hi_sqr > 3.841:
            print("Више критичного при p=0.05")
        else:
    ...
    def create_from_string(scr_str):
        ...
        if scr_str:
            ...
            print("Створюємо скремблер за степеннями:")
            for p in pow_list:
                print(p, end=" ")
            print("")
    ...
    def create_random(verbose = False):
        ...
        pows = random.sample(sample, random.randrange(1, len(sample) - 1))
        pows.sort(reverse=True)
        
        if verbose:
            print("Сгенеровано випадковий скремблер:")
            for p in pows:
                print(int(p), end=" ")
            print("")
    ...
    def demo():
        print("Демонстрація на умові із приклада:")
